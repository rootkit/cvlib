/*
    ...
    Bachelor's degree in Computer Systems Engineering 2016/2017
...
// Determines maximum RGB/BGR value
...
// Determines minimum RGB/BGR value
...
// Extracts RGB/BGR image red channel and converts respective channel to grayscale
...
// Extracts RGB/BGR image green channel and converts respective channel to grayscale
...
// Extracts RGB/BGR image blue channel and converts respective channel to grayscale
...
// Prints HSV values of each pixel of a given image
int vc_print_hsv_values(IVC *originalImage) 
{
	...
	for (i = 0; i < originalImage->width; i++)
	{
		...
		{
			posicao = (j * originalImage->bytesperline) + (i * originalImage->channels);
			printf("%d\t%d\t%d\n", originalImage->data[posicao], originalImage->data[posicao + 1], originalImage->data[posicao + 2]);
			k++;
		}
	}
...
// Converts grayscale image to a binary image through the use of a threshold
...
// Converts grayscale image to a binary image through the use of a global mean threshold
...
// Applies binary threshold to a HSV image
// 0 for a green threshold, 1 for a blue threshold, 2 for a colored bob with a minimum of saturation value
...
// Converts black pixels of a given grayscale image into white pixels
...
// Converts binary image to BGR through the use of a OpenCV data structure
...
// Converts RGB image to BGR through the use of a OpenCV data structure
...
// Applies binary dilation through variable kernel
...
// Applies binary erosion through variable kernel
...
// Applies binary close operation
int vc_binary_close(IVC *originalImage, IVC *destinationImage, int erodeSize, int dilateSize)
{
	IVC *imagemTemporaria;
	imagemTemporaria = vc_image_new(originalImage->width, originalImage->height, originalImage->channels, originalImage->levels);
	...
	if (vc_binary_dilate(originalImage, imagemTemporaria, dilateSize) == 0)
		return 0;

	if (vc_binary_erode(imagemTemporaria, destinationImage, erodeSize) == 0)
		return 0;
...
int vc_binary_open(IVC *originalImage, IVC *destinationImage, int erodeSize, int dilateSize)
{
	...
	if (vc_binary_erode(originalImage, imagemTemporaria, erodeSize) == 0)
		return 0;

	if (vc_binary_dilate(imagemTemporaria, destinationImage, dilateSize) == 0)
		return 0;
...
// Labelling of blobs of a given source binary image. The destination grayscale imagem will have the respective labels
...
// Fills blobs of a given image with they're respective information
...
// Prints information of a given blob
...
// Prints information of a given set of blobs
...
// Returns, from a given set of blobs, the label of the blob which haves the largest area 
...
// Verifies which blobs are inside of a boundary box of a given blob
int vc_is_blob_inside_interest_blob(OVC *segmentedImageBlobs, int interestBlobLabel, OVC *segmentedInvertedImageBlobs, int invertedBlobLabel)
{
	...
	if ((segmentedInvertedImageBlobs[invertedBlobLabel].x >= xMin && segmentedInvertedImageBlobs[invertedBlobLabel].x <= xMax)
		&& (segmentedInvertedImageBlobs[invertedBlobLabel].y >= yMin && segmentedInvertedImageBlobs[invertedBlobLabel].y <= yMax))
		return 1;
	else
		return 0;
}
...
// Counts and returns existing blobs inside of a given blob of interest
...
// Given a blob of interest in a segmented image, isolates the respective blob, pixel by pixel, in the inverted segmented image
...
// Draws boundary box of a given blob of a RGB image
...
// Draws boundary box of a given blob of a binary image
...
// Draws boundary box of a given blob of an BGR image through a OpenCV data structure
...
// Returns circularity of a given blob
...
// Determines largest area between red or blue segmentation resulting blobs
// 0 if the largest blob is a result of a a red segmentation, 1 if the largest blob is a result of a blue segmentation
...
// Counts and returns number of black pixels of each horizontal half of a boundary box
...
// Returns the number of black pixels of each vertical half of a boundary box
...
// Returns the side of a boundary box with the largest number of black pixels
...
// Draws boundary box on all blobs that exceed a minimum area passed by parameter
...
// Returns a boolean that represents if a blob of a given image was already checked
// Allows the insertion of a tolerance value in order to avoid slight changes in the blob area
...
// Adds new checked blob to the linked list of checked blobs
...
// Returns number of checked blobs contained in linked list
...
// Prints list of checked blobs. To be used for debugging purposes
...
// Frees memory associated with a particular list of checked blobs
...
// Draws crosshair in the center-of-mass of a given blob
// 0 for a green crosshair, 1 for a red crosshair
...
// Draws a crosshair in the center-of-mass of all blobs whose are is superior to a given value passed as parameter
// 0 for a green crosshair, 1 for a red crosshair
...
// Detects if an blob crosses the center of the image
...
// Given a grayscale image, returns image histogram through a integer array
...
// Returns largest number of pixels contained in the levels of a grayscale histogram
...
// Prints values of a grayscale histogram 
...
// Given a image and a histogram, returns a normalized histogram between 0 and 1 (probability density function)
...
// Returns largest probability density function value contained in a grayscale normalized histogram
...
// Prints grayscale histogram probability density function values
...
// Draws a white line in the horizontal center of a binary image. A pixel offset can be introduced as parameter
...
// Draws a white line in the horizontal center of a BGR image. A pixel offset can be introduced as parameter
